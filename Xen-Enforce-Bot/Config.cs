        ...
        public static void init(string filename) {
            ...
            if (!File.Exists(filename)) // Check to make sure the configuration file exists first
            {
                Helpers.writeOut(tag,
                    "Configuration file loading skipped. File doesn't exist. Assuming environment variables instead."); // if it doesn't , skip
                return; // return
            }

            try // make sure we're safe
            {
                var rom_contents = File.ReadAllLines(filename); // read the file, split at \r\n
                for (var i = 0; i < rom_contents.Length; i++) // for each line
                {
                    var gx = rom_contents[i]
                        .Split(splitarc, 2,
            ...
            {
                Helpers.writeOut(tag, "Error reading configuration {0}",
                    E.ToString()); // Throw error if something happens
            }
        }
